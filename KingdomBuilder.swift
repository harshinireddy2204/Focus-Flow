...
// MARK: - Offline On-Device AI + Advanced Markers/Citizens
...
class OnDeviceTaskAI: ObservableObject {
    ...
    func breakdownTask(_ input: String) async -> [String] {
        await MainActor.run {
            isThinking = true
            thinkingPhase = "Reading your task..."
        }

        #if canImport(FoundationModels)
        if #available(iOS 18.2, macOS 15.2, visionOS 2.2, *) {
            do {
                await MainActor.run { thinkingPhase = "Apple Intelligence is planning your steps..." }
                let session = LanguageModelSession(instructions: """
                You are a task breakdown specialist. Return only a JSON array of 5 to 9 short strings.
                """)
                let response = try await session.respond(to: "Break down this goal into practical steps: \(input)")
        ...
        await MainActor.run { thinkingPhase = "Using offline planner..." }
        let fallback = TaskAI.breakdownTask(input)
        await MainActor.run { isThinking = false }
        return fallback
    }
}
...
enum BuildingLevel: Int, Comparable {
    ...
    static func from(buildingsOwned: Int) -> BuildingLevel {
        switch buildingsOwned {
        case 0: return .foundation
        case 1...2: return .built
        case 3...5: return .upgraded
...
struct BuildingMarker: View {
    ...
    private var markerBody: some View {
        ...
        case .housing:
            VStack(spacing: 0) {
                TriangleRoof().fill(Color.brown).frame(width: 28, height: 14)
                RoundedRectangle(cornerRadius: 3).fill(Color(red: 0.9, green: 0.84, blue: 0.72)).frame(width: 30, height: 20)
            }
        case .economy:
            VStack(spacing: 0) {
                RoundedRectangle(cornerRadius: 2).fill(.orange).frame(width: 34, height: 8)
                RoundedRectangle(cornerRadius: 3).fill(Color(red: 0.88, green: 0.82, blue: 0.66)).frame(width: 30, height: 20)
                if isActiveSession { Text("💰").font(.system(size: 10)).offset(y: -30) }
            }
        case .culture:
            VStack(spacing: 0) {
                RoundedRectangle(cornerRadius: 4).fill(.purple.opacity(0.9)).frame(width: 26, height: 10)
                RoundedRectangle(cornerRadius: 4).fill(Color(red: 0.82, green: 0.78, blue: 0.92)).frame(width: 32, height: 22)
            }
        case .defense:
            VStack(spacing: 0) {
                RoundedRectangle(cornerRadius: 2).fill(Color.gray).frame(width: 34, height: 8)
                RoundedRectangle(cornerRadius: 3).fill(Color(red: 0.56, green: 0.54, blue: 0.52)).frame(width: 30, height: 22)
            }
        case .nature:
            ZStack {
                Ellipse().fill(Color(red: 0.2, green: 0.65, blue: 0.28)).frame(width: 32, height: 14).offset(y: 12)
                Circle().fill(Color(red: 0.16, green: 0.6, blue: 0.2)).frame(width: 20, height: 20).offset(y: -2)
                Rectangle().fill(Color(red: 0.45, green: 0.28, blue: 0.12)).frame(width: 4, height: 10).offset(y: 6)
            }
        }
    }
}

enum CitizenState { case idle, cheering, sad }
...
class CitizenManager: ObservableObject {
    ...
    func setupCitizens(population: Int, width: CGFloat, height: CGFloat) {
        let count = min(population, 14)
        let emojis = ["🧑","👩","🧒","👴","👵","🧑‍💼","👨‍🎓","👩‍🎓"]
        citizens = (0..<count).map { i in
            let xBase = width * (0.16 + CGFloat(i % 5) * 0.16)
    ...
    func onTaskCompleted(width: CGFloat) {
        for i in citizens.indices { citizens[i].state = .cheering }
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            for i in self.citizens.indices {
                self.citizens[i].state = .idle
                let nx = self.citizens[i].x + CGFloat.random(in: -16...16)
                self.citizens[i].targetX = max(20, min(width - 20, nx))
            }
        }
    }
}
    ...
    var body: some View {
        ForEach(manager.citizens) { citizen in
            ZStack {
                ...
                if citizen.state == .cheering { Text("⭐").font(.system(size: 8)).offset(y: -12) }
            }
            .position(x: citizen.targetX, y: citizen.y + (bob ? 2 : -2))
            .animation(.easeInOut(duration: 0.8), value: citizen.targetX)
...
struct BuildingEvolutionCard: View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                ...
                Text(levelLabel)
                    ...
                    .background(Capsule().fill(type.category.color.opacity(0.12)))
            }
            Text("Own more \(type.name)s to evolve visuals: Plot → Built → Upgraded → Legendary")
                .font(.caption).foregroundColor(.secondary)
            ProgressView(value: progress)
                .tint(type.category.color)
        }
        .padding(16)
    ...
    private var progress: Double {
        let thresholds = [0, 1, 3, 6]
        let next = min(level.rawValue + 1, 3)
        if level == .legendary { return 1 }
        let prevT = thresholds[level.rawValue]
        let nextT = thresholds[next]
        return min(1, max(0, Double(owned - prevT) / Double(nextT - prevT)))
    }

    private var levelLabel: String {
        switch level {
        case .foundation: return "Plot"
...
struct KingdomView: View {
    ...
    @StateObject private var citizenManager = CitizenManager()
    var skyColors: [Color] {
        ...
        GeometryReader { geo in
            ...
            ZStack {
                ...
                ForEach(ShopCategory.allCases, id: \.rawValue) { cat in
                    ...
                    ForEach(Array(catBuildings.enumerated()), id: \.element.id) { idx, building in
                        ...
                        Button(action: { kingdom.selectedBuilding = building }) {
                            BuildingMarker(type: building.type, category: cat, buildingsOwned: kingdom.buildingsOfType(building.type), isActiveSession: kingdom.tasks.contains { !$0.completed })
                        }
                ...
                if kingdom.population > 0 {
                    ReactiveCitizensView(manager: citizenManager)
                        .accessibilityHidden(true)
                }
            ...
            .onAppear {
                ...
                baseZoomScale = zoomScale
                citizenManager.setupCitizens(population: kingdom.population, width: w, height: h)
            }
...
struct TaskInputSheet: View {
    ...
    @StateObject private var onDeviceAI = OnDeviceTaskAI()
    @State private var thinkingPhase = ""
        ...
        Task {
            for i in 1...20 {
                ...
                await MainActor.run {
                    ...
                    thinkingPhase = onDeviceAI.thinkingPhase
                }
            }
